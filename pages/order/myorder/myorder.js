...
Page({
    ...
    getData(){
        ...
        console.log(_active);
        util.request(api.OrderList, { status: _active,page:that.data.page,size:that.data.size}).then(function (res) {
            console.log(res);
            that.setData({
    ...
    viewExpress(e){  // 查看物流信息
        console.log(e.currentTarget.dataset);
        let _id = e.currentTarget.dataset;
        wx.navigateTo({
            url: '../express/express?orderId=' + e.currentTarget.dataset.orderid,
        })
    },
    goPay(e){  // 去支付
        console.log(e.currentTarget.dataset);
    ...
    onReachBottom: function () {
        ...
        if (that.data.currentPage < that.data.totalPages) {
            ...
            let _active = that.data.active;
            util.request(api.OrderList, { status: _active, page: nextPage, size: that.data.size }).then(function (res) {
                console.log(res);
